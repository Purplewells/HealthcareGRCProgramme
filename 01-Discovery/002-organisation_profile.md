# Organisation Profile  
## Westbridge Hospitals Trust (WHT)

**Project:** Project SentinelCare - Cyber Security Governance, Risk and Compliance Improvement Programme  
**Document Type:** Organisation Profile  
**Version:** 1.0  

# 1. Introduction

Westbridge Hospitals Trust (WHT) is a fictional NHS healthcare organisation created for portfolio purposes. The Trust represents a large, digitally enabled healthcare provider operating within the NHS environment and serving communities across the West Midlands region.

The Trust delivers a wide range of acute, specialist, diagnostic, and community healthcare services. As a modern healthcare provider, WHT relies heavily on digital technology to support clinical decision-making, improve patient outcomes, enable efficient service delivery, and maintain continuity of care.

The increasing dependence on digital systems has created a complex cyber security landscape. Clinical systems, medical devices, cloud services, third-party suppliers, and patient-facing platforms must operate securely while remaining available to healthcare professionals when needed.

Following increasing concerns about cyber threats affecting healthcare organisations nationally, the Trust Board has commissioned **Project SentinelCare** to assess and improve its cyber security governance, risk management, and compliance capabilities.

# 2. Organisation Overview

Westbridge Hospitals Trust provides acute and specialist healthcare services across the West Midlands region.

The Trust operates a combination of hospital-based and community healthcare facilities, supporting both routine and emergency healthcare delivery.

The organisation operates:

- Two acute hospital sites
- Three community healthcare centres
- Specialist cancer treatment services
- Emergency department services
- Diagnostic imaging departments
- Pathology laboratories
- Pharmacy services
- Research and clinical trial facilities

The Trust provides healthcare services to approximately:

| Service Area | Annual Volume |
|---|---:|
| Patients Served | 850,000 |
| Outpatient Appointments | 2.5 million |
| Emergency Admissions | 180,000 |

Due to the scale and complexity of its operations, maintaining secure and resilient digital services is critical to patient safety and operational continuity.

As an NHS Foundation Trust, WHT is regulated by the Care Quality Commission (CQC) and works within the wider NHS oversight structure alongside its local Integrated Care Board (ICB). Foundation Trust status also brings a Council of Governors, representing patients, staff, and the public in the Trust's governance (see [006-stakeholders](006-stakeholders.md) for full stakeholder detail).

# 3. Mission and Strategic Purpose

The Trust's mission is to provide safe, effective, and compassionate healthcare supported by secure and reliable digital services.

WHT aims to:

- Deliver high-quality patient care.
- Improve healthcare outcomes through digital innovation.
- Enable secure information sharing across healthcare providers.
- Protect confidential patient and staff information.
- Maintain resilient clinical services.
- Support research and healthcare innovation.

Cyber security is recognised as an essential enabler of the Trust's strategic objectives rather than solely an information technology responsibility.

# 4. Organisational Workforce

The Trust employs approximately **8,500 staff members** across clinical, operational, research, administrative, and technology functions.

| Workforce Group | Approximate Number |
|---|---:|
| Clinical Staff | 1,200 |
| Administrative Employees | 1,000 |
| Contractors | 300 |
| Students and Researchers | 400 |
| Other Healthcare and Support Staff | 6,600 |

The diverse workforce introduces different levels of cyber security exposure. Clinical professionals require rapid access to patient information, while administrative, research, and support teams interact with sensitive systems and data.

The Trust recognises that effective cyber security requires both technical controls and a strong security culture across all workforce groups.

# 5. Healthcare Services

## Acute Healthcare Services

The Trust provides emergency and specialist hospital care, including:

- Emergency department services
- Inpatient care
- Surgical services
- Critical care
- Specialist medical services

These services depend on continuous availability of clinical systems, communication platforms, and supporting infrastructure.

## Specialist Services

WHT provides specialist healthcare services including:

- Cancer treatment services
- Specialist outpatient clinics
- Clinical research programmes
- Advanced diagnostic services

These areas process highly sensitive patient information and often involve complex technology environments.


## Diagnostic Services

Diagnostic services include:

- Radiology imaging
- Pathology laboratories
- Clinical testing services
- Diagnostic reporting

These services depend on integrated systems that exchange clinical information between departments and healthcare professionals.


## Pharmacy Services

The Trust operates pharmacy services supporting:

- Medication management
- Electronic prescribing
- Drug safety monitoring
- Clinical decision support

Security and availability of pharmacy systems are critical to maintaining patient safety.


# 6. Digital Transformation Journey

Over the past five years, WHT has undergone significant digital transformation to improve healthcare delivery and patient experience.

Key digital initiatives include:

- Implementation of Electronic Patient Record (EPR) capabilities.
- Expansion of patient-facing digital services.
- Increased adoption of cloud technologies.
- Integration of healthcare systems.
- Introduction of remote consultation services.
- Improved data analytics capabilities.

While digital transformation has improved efficiency and accessibility, it has also increased the Trust's cyber security responsibilities.

The expanded digital estate has introduced additional risks including:

- Increased attack surface.
- Greater dependency on third-party suppliers.
- More complex identity and access requirements.
- Increased volumes of sensitive healthcare data.
- Greater dependency on system availability.


# 7. Digital Healthcare Environment

The Trust operates a hybrid technology environment consisting of cloud services, on-premises infrastructure, clinical applications, and connected medical devices.

## Clinical Systems

- Electronic Patient Record (EPR)
- Laboratory Information System (LIS)
- Radiology Information System (RIS)
- Picture Archiving and Communication System (PACS)
- Pharmacy Management System
- Theatre Management System

## Patient Digital Services

- Online patient portal
- Appointment booking platform
- Remote consultation services
- Patient messaging platform

## Corporate Systems

- Microsoft 365
- Finance and procurement platforms
- Human Resources and payroll systems
- Learning management platform

## Infrastructure

- Hybrid cloud environment
- Microsoft Azure workloads
- On-premises data centres
- Clinical network infrastructure
- Connected medical devices
- Third-party healthcare applications

# 8. Information Assets

As a healthcare provider, WHT manages a wide range of sensitive information assets.

## Patient Information

- Electronic patient records
- Diagnoses
- Treatment information
- Medication records
- Laboratory results
- Medical images
- Appointment information

## Staff Information

- Employee records
- Payroll information
- Training records
- Workforce management data

## Research Information

- Clinical trial information
- Research datasets
- Healthcare innovation data

## Operational Information

- Financial records
- Procurement information
- Supplier information
- Business continuity documentation

The confidentiality, integrity, and availability of these assets are essential to maintaining patient trust and safe healthcare delivery.

# 9. Stakeholder Landscape

Cyber security governance requires collaboration across executive, clinical, digital, governance, and external functions of the Trust. The full stakeholder register, including roles, responsibilities, and level of interest in the programme, is maintained in [006-stakeholders](006-stakeholders.md).


# 10. Cyber Security Context

Healthcare organisations are increasingly targeted by cyber threats due to the value of health information and the critical nature of healthcare services.

The Trust faces several cyber security challenges:

- Phishing and credential theft.
- Ransomware threats.
- Legacy medical technology.
- Supplier security risks.
- Excessive access privileges.
- Data governance challenges.
- Limited security visibility.

These challenges have driven the requirement for Project SentinelCare.

# 11. Project SentinelCare Context

Project SentinelCare has been commissioned to improve the Trust's cyber security maturity by establishing stronger governance, risk management, and compliance processes.

The programme will support the Trust in:

- Understanding its cyber risk exposure.
- Improving regulatory compliance.
- Strengthening security governance.
- Improving incident preparedness.
- Enhancing executive reporting.
- Building a culture of security awareness.

The expected outcome is a more resilient healthcare organisation capable of protecting patients, clinical services, and sensitive information against evolving cyber threats.